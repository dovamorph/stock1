...
# ════════════════════════════════════════════════════════════════════════
# ⑥ 포지션 사이징 (베타 + ATR + 점수 통합)
# ════════════════════════════════════════════════════════════════════════
def calc_position_size(
    base_capital:   float,   # 장투 350만 / 단타 150만
    ...
    total = math.floor(base_capital * effective_mult * beta_m * atr_m / 1000) * 1000
    total = max(0, total)
    ...
    return {
        ...
        "beta_mult":   beta_m,
        "atr_mult":    atr_m,
        "desc": (f"총{total//10000}만 | 1차{first//10000}만({int(fr*100)}%) | "
                 f"β{beta:.1f}(×{beta_m}) ATR{atr_pct*100:.1f}%(×{atr_m})")
    }
...
def analyze_before_buy(
    ...
    반환:
    {
      ...
      "atr_pct":   float,
      "beta":      float,
      "sizing":    dict,    # calc_position_size 결과
    }
    """
    ...
    # 거래량 비율 (20일 평균 대비)
    vol_20avg = (sum(o["volume"] for o in ohlcv[1:21]) / min(len(ohlcv)-1, 20)
    ...
    print(f"    [{name}] 점수:{score}점({score_res['strength']}) | "
          f"RSI:{rsi:.0f} 외인:{'+' if foreign_net>0 else ''}{foreign_net:,} "
          f"ATR:{atr_pct*100:.1f}% 거래량:{vol_ratio:.1f}x")

    if score < min_score:
        return {"ok": False, "reason": f"매수점수 부족 ({score}점 < {min_score}점)",
                "score": score, "strength": score_res["strength"]}
    ...
    if sizing["total"] <= 0:
        return {"ok": False, "reason": "포지션 금액 0원", "score": score,
...
if __name__ == "__main__":
    ...
    for c in cases:
        ...
        print(f"  {c['grade']}등급 RSI{c['rsi']} → {r['score']}점 [{r['strength']}] "
              f"1차비중:{int(r['first_ratio']*100)}%  {r['detail']}")
    ...
    for beta in [0.7, 1.0, 1.3, 1.8]:
        r = calc_position_size(3_500_000, 0.9, 7, beta=beta, atr_pct=0.02)
        print(f"  β{beta} → 총{r['total']//10000}만 1차{r['first']//10000}만 | {r['desc']}")
